...
def save_holiday():
    ...
    holidays_obj = Holidays.query.filter_by(bdate=date).first()
    if reservation_obj and holidays_obj:
...
def save_user():
    name, address, email_address, phone_no, user_role = request.form['name'], request.form['address'], request.form['email_address'], request.form['phone_no'], request.form['user_role']
    user_entry = Users(
        name=name,
        pwd='password',
        address=address,
...
def option(option):
    if 'email' in session:
        ...
        return render_template('dashboard.html', selected_option=option, current_month_data=current_month_data, users_all_obj=users_all_obj, holidays_all_obj=holidays_all_obj,reservations_all_obj=reservations_all_obj, events_and_user_combined=events_and_user_combined)
